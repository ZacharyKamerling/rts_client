class UnitDrawer {
    ...
    public draw(x: number, y: number, scale: number, sprites: { x: number, y: number, ang: number, ref: string }[]) {
        ...
        for (let i = 0, n = 0; n < sprites.length; n++) {
            ...
            let xywh = this.spriteMap.coords(sprites[n].ref);
            ...
            let ne = UnitDrawer.rotateAroundOrigin(normX, normY, east, north, sprite.ang);
            let sw = UnitDrawer.rotateAroundOrigin(normX, normY, west, south, sprite.ang);
            let nw = UnitDrawer.rotateAroundOrigin(normX, normY, west, north, sprite.ang);
            let se = UnitDrawer.rotateAroundOrigin(normX, normY, east, south, sprite.ang);
