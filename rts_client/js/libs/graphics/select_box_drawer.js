var SelectBoxDrawer = (function () {
    function SelectBoxDrawer(canvas) {
        this.canvas = canvas;
        var gl = this.canvas.getContext('webgl');
        this.program = new MetaProgram(gl, createProgram(gl, SelectBoxDrawer.vertexShader, SelectBoxDrawer.fragmentShader));
        this.buffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, this.buffer);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array([]), gl.STATIC_DRAW);
    }
    SelectBoxDrawer.prototype.draw = function (x, y, scale, circles) {
        x = Math.floor(x);
        y = Math.floor(y);
        scale = scale / 4;
    ...
    SelectBoxDrawer.fragmentShader = [
        "precision highp float;",
        ...
        "    if (dist <= (v_circle_radius * v_circle_radius) && dist >= ((v_circle_radius - 0.001) * (v_circle_radius - 0.001))) {",
